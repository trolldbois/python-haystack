...
# QFont, QFontMetrics, QMenu, QClipboard, QApplication, QFontDialog
...
class QHexeditWidget(QtGui.QAbstractScrollArea):
    ...
    def format_address(self, address, showSep=False):
        ...
        if s == 4:
            return QString("%04x%s%04x" %
                           ((address >> 16) & 0xffff, sep, address & 0xffff))
        elif s == 8:
            return QString(
                "%08x%s%08x" % ((address >> 32) & 0xffffffff, sep, address & 0xffffffff))
        return
  ...
  '''
    ...
    def createStandardContextMenu(self):
        ...
        menu.addSeparator()
        self.add_toggle_action_to_menu(
            menu,
            tr("Show A&ddress"),
            self.show_address,
            self.setShowAddress)
        ...
        a1 = self.add_toggle_action_to_menu(
            wordMenu,
            tr("1 Byte"),
            self.word_width == 1,
            wordWidthMapper.map)
        a2 = self.add_toggle_action_to_menu(
            wordMenu,
            tr("2 Bytes"),
            self.word_width == 2,
            wordWidthMapper.map)
        a3 = self.add_toggle_action_to_menu(
            wordMenu,
            tr("4 Bytes"),
            self.word_width == 4,
            wordWidthMapper.map)
        a4 = self.add_toggle_action_to_menu(
            wordMenu,
            tr("8 Bytes"),
            self.word_width == 8,
            wordWidthMapper.map)
        ...
        a5 = self.add_toggle_action_to_menu(
            rowMenu,
            tr("1 Word"),
            self.row_width == 1,
            rowWidthMapper.map)
        a6 = self.add_toggle_action_to_menu(
            rowMenu,
            tr("2 Words"),
            self.row_width == 2,
            rowWidthMapper.map)
        a7 = self.add_toggle_action_to_menu(
            rowMenu,
            tr("4 Words"),
            self.row_width == 4,
            rowWidthMapper.map)
        a8 = self.add_toggle_action_to_menu(
            rowMenu,
            tr("8 Words"),
            self.row_width == 8,
            rowWidthMapper.map)
        a9 = self.add_toggle_action_to_menu(
            rowMenu,
            tr("16 Words"),
            self.row_width == 16,
            rowWidthMapper.map)
  ...
  '''
    ...
    def updateScrollbars(self):
        ...
        self.verticalScrollBar().setMaximum(
            max(0, sz / bpr + horn - self.viewport().height() / self.font_height))
        self.horizontalScrollBar().setMaximum(
            max(0, (self.line3() - self.viewport().width()) / self.font_width))
        return
  ...
  '''
    ...
    def setData(self, data):
        if not isinstance(data, QIODevice) and not isinstance(
                data, QByteArray):
            # transform it
        ...
        if index < self.dataSize():
            if self.selection_start != self.selection_end:
                if self.selection_start < self.selection_end:
                    ...
                    ret = (
                        index >= self.selection_end and index < self.selection_start)
        return ret
...
def argparser():
    rootparser = argparse.ArgumentParser(
        prog='haystack-gui',
        description='Hexedit widget demo.')
