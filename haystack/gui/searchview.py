...
class SearchStructDialog(QtGui.QDialog, Ui_Search_Structure):
    ...
    def search_regexp(self):
        ...
        log.debug(
            'Looking for a regexp r"%s" ' %
            (self.lineEdit_regexp.text()))
        tab = self.parent().currentTab()
        if tab is not None:
            items = tab.search_regexp(
                str(self.lineEdit_regexp.text()), str(self.lineEdit_regexp.text())[-20:])
    ...
    def fillTree(self):
        ...
        for mod in mods:
            ...
            for cls, typ in inspect.getmembers(mod, inspect.isclass):
                ...
                if typ.__module__.startswith(
                        mod.__name__) and ctypes.is_struct_type(typ()):
