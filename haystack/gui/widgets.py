...
class Structure(QtGui.QGraphicsItemGroup):
    ...
    def __init__(
            self, offset, value, color=QtCore.Qt.green, scene=None, parent=None):
        ...
        log.debug(
            'hover: %s handleChildEvents: %s' %
            (self.acceptsHoverEvents(), self.handlesChildEvents()))
    ...
    def _makeRectItem(self):
        ...
        rectItem = QtGui.QGraphicsRectItem(
            parent=self)  # no need to add to scene
    ...
    def _makeStruct(self, offset, size):
        ...
        if (x1 + size > width):
            ...
            return
        # MIDDLE LINES
        # then draw big rect full lines from ya = y+1  to yb = ((offset+size)
        # // width) - 1
...
class Word(Structure):
    ...
    def __init__(
            self, offset, value, color=QtCore.Qt.yellow, scene=None, parent=None):
        QtGui.QGraphicsItemGroup.__init__(self, parent, scene)
        self.scene = scene
        self.value = value
        self.offset = offset
