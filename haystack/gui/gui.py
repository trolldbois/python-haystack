...
class MemoryMappingWidget(QtGui.QWidget, Ui_MemoryMappingWidget):
    ...
    def searchNullWords(self):
        ...
        for k, g in itertools.groupby(
                enumerate(tmpnull), lambda i_x: i_x[0] - i_x[1]):
    ...
    def searchStructure(
            self, structType='sslsnoop.ctypes_openssh.session_state'):
        '''
        ...
        if len(instances) > 0:
            log.debug(
                'received %d struct of size %d' %
                (len(instances), len(
                    instances[0][0])))
        # init graphical element
        resultsViewer = searchinfoview.SearchInfoView(
            self.scene,
            QtCore.Qt.green,
            withDetails=True,
            parent=self.tab_search_structures)
    ...
    def search_regexp(self, regexp, searchName, color=QtCore.Qt.black):
        ...
        # add a entry into the tabView on the right so we can play with it more
        # easily
        resultsViewer = searchinfoview.SearchInfoView(
            self.scene,
            color,
    ...
    def show_structures_allocated(self, context):
        ...
        for s in context.listStructures():
            ...
            self.allocated_structures.addToGroup(
                widgets.Structure(
                    offset(
                        s._vaddr),
                    s,
...
class MyMain(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def setupUi(self, me):
        ...
        self.connect(
            ...
            self.openDump)
        self.connect(
            self.menu_file_close,
            QtCore.SIGNAL('triggered()'),
            self.closeTab)
        self.connect(
            self.menu_search_structure,
            QtCore.SIGNAL('triggered()'),
            self.dialog_searchStructure)
        self.connect(
            self.menu_tools_addmodule,
            QtCore.SIGNAL('triggered()'),
            self.dialog_addModule)
